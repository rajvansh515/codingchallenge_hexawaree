    ...
    public class DatabaseManagerRepository:IDatabaseManagerRepository
    {
        ...
        public void InsertJobListing(JobListing job)
        {
            using (SqlConnection connection = new SqlConnection(_connectionString))
            {
                ...
                string companyCheckQuery = "SELECT COUNT(*) FROM Companies WHERE CompanyID = @CompanyID";
                SqlCommand companyCheckCommand = new SqlCommand(companyCheckQuery, connection);
                companyCheckCommand.Parameters.AddWithValue("@CompanyID", job.CompanyID);

                int companyExists = (int)companyCheckCommand.ExecuteScalar();

                if (companyExists == 0)
                {
                    throw new Exception($"CompanyID {job.CompanyID} does not exist in the Companies table.");
                }

                // If the company exists, insert the job listing
                string insertQuery = @"
